...
const MidCap_Info_Modal = ({ isOpen, onClose }) => {
    ...
    const fileUpload = async (e) => {
        ...
        if (fileRead) {
            ...
            formData.append('mid_ExcelSheet', fileRead);
            try {
                const response = await axios.post('/api/v1/mid_excelRead', formData, {
                    ...
                    <form action="POST" className='space-y-4'>
                        <div className='flex gap-2'>
                            <div className='w-1/2 flex items-end gap-0.5'>
                                <div>
                                    <label htmlFor="mid_ExcelSheet">File Upload</label>
                                    <input className='p-0.5 w-full' type="file" id='mid_ExcelSheet' name='mid_ExcelSheet' accept='.txt' onChange={fileOnchange} />
...
export default MidCap_Info_Modal
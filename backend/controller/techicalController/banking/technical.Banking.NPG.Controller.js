...
exports.techicalBankDataToCreateTable_NPG = async (req, resp) => {
    try {
        const filePath = req.file.path;
        const { results, DynamicModel } = await readerFileService.createDynamicTableNPG(filePath);
        if (results.length > 0) {
...
//  GET
exports.getAllTechnicalBankData_NPG = async (req, res) => {
    try {
        ...
        const collection = mongoose.connection.db.collection('techicalbankingnpgs');
...
// DELETE
exports.deleteTechnicalBankById_NPG = async (req, resp) => {
...
exports.deleteOrTruncateTable = async (req, resp) => {
    ...
    try {
        ...
        const collection = db.collection('techicalbankingnpgs'); // Ensure the collection name matches exactly
